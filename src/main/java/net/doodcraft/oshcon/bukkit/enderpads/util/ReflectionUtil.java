...
public class ReflectionUtil {
    ...
    public static void sendActionBar(Player player, String message) {
        if (!SUPPORT183_1112) {
            if (Compatibility.isSupported(PadsPlugin.version, "1.8.3", "1.11.2")) {
                SUPPORT183_1112 = true;
            }
        }
        if (!SUPPORT112_1122) {
            if (Compatibility.isSupported(PadsPlugin.version, "1.12", "1.12.2")) {
    ...
    private static void send18Actionbar(Player player, String message) {
        try {
            ...
            player.sendMessage(PadsPlugin.logger.addColor(message));
            if (Settings.debug) {
                ex.printStackTrace();
            }
        }
